...
public class TabLayout extends HorizontalScrollView {
    ...
    private IDesignLayoutTabTextLayout designLayoutTabTextLayoutImpl;

    public IDesignLayoutTabTextLayout getDesignLayoutTabTextLayoutImpl() {
        return designLayoutTabTextLayoutImpl;
    }

    public void setDesignLayoutTabTextLayoutImpl(IDesignLayoutTabTextLayout designLayoutTabTextLayoutImpl) {
        this.designLayoutTabTextLayoutImpl = designLayoutTabTextLayoutImpl;
    }
        ...
        if (startScrollX != targetScrollX) {
            ...
            mScrollAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    // mSelectedTab.mView.mCustomTextView

                    //textView.setTextSize(16 + (22 - 16) * (float)animation.getAnimatedValue());
                }
            });
        }
    ...
    public class TabView extends LinearLayout {
        ...
        public TextView getmCustomTextView() {
            return mCustomTextView;
        }

        public void setmCustomTextView(TextView mCustomTextView) {
            this.mCustomTextView = mCustomTextView;
        }
        ...
        public boolean performClick() {
            ...
            if (mTab != null) {
                ...
                //executeTextAnim(mTab.mView.mCustomTextView, mTabStrip, mTab.getPosition(),mTab);
                // mTabss = mTab;
                mTab.select();
        ...
        final void update() {
            ...
            if (mCustomView == null) {
                ...
                if (mTextView == null) {
                    TextView textView;
                    if (designLayoutTabTextLayoutImpl != null) {
                        textView = (TextView) LayoutInflater.from(getContext())
                                .inflate(designLayoutTabTextLayoutImpl.getDesignLayoutTabTextLayout(), this, false);
                    } else {
                        textView = (TextView) LayoutInflater.from(getContext())
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        //设置底部线的宽度
        private int mSelectedIndicatorWidth = dpToPx(1);
            ...
            if (startLeft != targetLeft || startRight != targetRight) {
                ...
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animator) {
                        ...
                        if (mTabss != null) {
                            // mTabss.mView.mCustomTextView.setTextSize((float) (16 + (22 - 16) * (float)animator.getAnimatedValue()));
                        }
                ...
                if (tabLayout.mNeedSwitchAnimation) {
                    if (mScrollState == SCROLL_STATE_SETTLING && mPreviousScrollState == SCROLL_STATE_IDLE) {
                       ...
                       /* if (tabLayout.mTabStrip.mSelectedPosition + 1 < tabLayout.mTabStrip.getChildCount()) {
                            ...
                            //执行文字变大变小 22为最大字体，16为最小字体
                            //Tab tab = tabLayout.getTabAt(position);
                            //Tab nextTab = tabLayout.getTabAt(position + 1);
                            //if (tab != null && nextTab != null) {
                            tab.mView.mCustomTextView.setTextSize(22 - (22 - 16) * positionOffset);
                            //nextTab.mView.mCustomTextView.setTextSize(16 + (22 - 16) * positionOffset);
                            // }
    ...
    public class ViewPagerOnTabSelectedListener implements OnTabSelectedListener {
        ...
        public void onTabSelected(Tab tab) {
            ...
            //执行动画
            // executeTextAnim(tab.mView.mCustomTextView);
    ...
    //tab点击动画，如果不滑动就没有偏移量可做
    public void executeTextAnim(final TextView textView, final SlidingTabStrip mTabStrip, int position,
                                final Tab mTab) {

        final int var = position;
        final AccelerateInterpolator mStartInterpolator = new AccelerateInterpolator();
        final DecelerateInterpolator mEndInterpolator = new DecelerateInterpolator(1.6f);
        final TabLayout tabLayout = TabLayout.this;
        if (textView == null) return;
        int size = (int) textView.getTextSize();
        ValueAnimator valueAnimator = new ValueAnimator();

        if (position > mTabStrip.mSelectedPosition) {
            valueAnimator.setFloatValues(0, 1);
        } else {
            valueAnimator.setFloatValues(1, 0);
        }
        if (position > currentPosition) {
            mTabStrip.mSelectedPosition = position - 1;
        } else {

            mTabStrip.mSelectedPosition = position;
        }
        currentPosition = position;

        if (mTabStrip.mSelectedPosition <= 0) {
            mTabStrip.mSelectedPosition = 0;
            valueAnimator.setFloatValues(0, 1);
        }
        ...
        valueAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                mSelectedTab = mTab;
                //mTabStrip.mSelectedPosition = var;
                mTabStrip.mSelectionOffset = 0f;
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        mViewPager.setCurrentItem(mTab.mPosition);
                    }
                }, 1000);
            }
    ...
    //Tab布局回掉接口
    public interface IDesignLayoutTabTextLayout {
        int getDesignLayoutTabTextLayout();
    }
