...
public class AppUtils {
    ...
    /*public static void setUUid() {
        String uuid = (String) SharedPreferencesUtils.getParam(MainApp.getInstance().context, "mapgo_uuid", "");
        if (uuid == null || uuid.length() == 0) {
            uuid = java.util.UUID.randomUUID().toString();
            SharedPreferencesUtils.setParam(MainApp.getInstance().context, "mapgo_uuid", uuid);
