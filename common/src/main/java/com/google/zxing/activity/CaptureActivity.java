...
//            this.setResult(RESULT_CODE_QR_SCAN, resultIntent);
            ...
            if (result.getText().contains("mapgo") && result.getText().contains("type") && result.getText().contains("userId")) {
                JSONObject jsonObject = JSONObject.parseObject(result.getText());
                String appName = jsonObject.getString(QRUtils.QR_APPNAME);
                String type = jsonObject.getString(QRUtils.QR_TYPE);
                String userId = jsonObject.getString(QRUtils.QR_USERID);
                if (appName.equals("mapgo")) {
                    switch (type) {
                        case "1":
                            if (!TextUtils.isEmpty(userId)) {
//                                Navigation.startOthersInfo(Long.parseLong(userId), "", 1);
                                Bundle bundle = new Bundle();
                                bundle.putLong("userId", Long.parseLong(userId));
                                bundle.putString("userNick", "");
                                bundle.putInt("type", 1);
                                IntentRoute.getIntentRoute().withType(ARouteHandleType.CODE_GOTO_OTHER).withExtra(userId).withExtra2("1")
                                        .navigation();
                            }
