...
public class PictureImageGridAdapter_CameraRoll extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public PictureImageGridAdapter_CameraRoll(Context context, PictureSelectionConfig config) {
        ...
        if (getItemViewType(position) == PictureConfig.TYPE_CAMERA) {
            ...
            if (enablePreview || enablePreviewVideo || enablePreviewAudio) {
                contentHolder.ll_check.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (PictureFileUtils.isDownLoading(path)) {
                            ToastUtils.showLong(context.getResources().getText(R.string.picture_error_suffix), context.getApplicationContext());
                            return;
                        }
    ...
    private void changeCheckboxState(ViewHolder contentHolder, LocalMedia image) {
        ...
        if (selectImages.size() == config.maxSelectNum) {
            isNotifyDataSetChangedCameraRoll = true;
            notifyDataSetChanged();
        } else {

            if (isNotifyDataSetChangedCameraRoll) {

                notifyDataSetChanged();
                isNotifyDataSetChangedCameraRoll = false;
            }
        }
    }

    private boolean isNotifyDataSetChangedCameraRoll = false;



    public boolean isNotifyDataSetChangedCameraRoll() {
        return isNotifyDataSetChangedCameraRoll;
    }

    public void setNotifyDataSetChangedCameraRoll(boolean notifyDataSetChangedCameraRoll) {
        isNotifyDataSetChangedCameraRoll = notifyDataSetChangedCameraRoll;
    }
        ...
        if (isNotifyDataSetChangedCameraRoll) {
            if (isChecked) {
                holder.iv_picture.setColorFilter(ContextCompat.getColor
                        (context, R.color.transparent), PorterDuff.Mode.SRC_ATOP);
    ...
    private OnSurplusPicIsShadowListener mOnSurplusPicIsShadowListener;

    public OnSurplusPicIsShadowListener getOnSurplusPicIsShadowListener() {
        return mOnSurplusPicIsShadowListener;
    }

    public void setOnSurplusPicIsShadowListener(OnSurplusPicIsShadowListener mOnSurplusPicIsShadowListener) {
        this.mOnSurplusPicIsShadowListener = mOnSurplusPicIsShadowListener;
    }

    public interface OnSurplusPicIsShadowListener {
        boolean isShadow();
    }
