...
public class ClientGlobal {
    ...
    public static boolean g_custom_storage_upload = false;
    public static String g_custom_storage_upload_host;
    public static int g_custom_storage_upload_port;

    public static boolean g_custom_storage_download = false;
    public static String g_custom_storage_download_host;
    public static int g_custom_storage_download_port;
    public static String g_custom_storage_download_prefix;
    ...
    public static void init() throws IOException, MyException {
        init(FastDFSConfigure.DevConfig.tracker_server_host, FastDFSConfigure.DevConfig.tracker_server_port
                , FastDFSConfigure.DevConfig.tracker_http_port, FastDFSConfigure.DevConfig.storage_for_upload_host
                , FastDFSConfigure.DevConfig.storage_for_upload_port, FastDFSConfigure.DevConfig.storage_for_download_host
                , FastDFSConfigure.DevConfig.storage_for_download_port,FastDFSConfigure.DevConfig.storage_for_download_prefix);
    }

    public static void init(String trackerServerHost, int trackerServerPort, int trackerHttpPort,
                            String storageUploadHost, int storageUploadPort
            , String storageDownloadHost, int storageDownloadPort,String customStoragePrefixForDownload) throws IOException, MyException {
//        String[] szTrackerServers;
//        String[] parts;
        g_connect_timeout = FastDFSConfigure.DevConfig.connect_timeout;
        g_connect_timeout *= 1000; //millisecond

        g_network_timeout = FastDFSConfigure.DevConfig.network_timeout;
        g_network_timeout *= 1000; //millisecond

        g_charset = FastDFSConfigure.DevConfig.charset;

//        szTrackerServers = new String[]{trackerServer};

        InetSocketAddress[] tracker_servers = new InetSocketAddress[1];
//        for (int i = 0; i < szTrackerServers.length; i++) {
//            parts = szTrackerServers[0].split("\\:", 2);
//            if (parts.length != 2) {
//                System.out.println("szTrackerServers[i] = " + szTrackerServers[0]);
//                tracker_servers[i] = new InetSocketAddress(szTrackerServers[i], 0);
//                throw new MyException("the value of item \"tracker_server\" is invalid, the correct format is host:port");
//            } else {
//                tracker_servers[i] = new InetSocketAddress(parts[0].trim(), Integer.parseInt(parts[1].trim()));
//            }
//            System.out.println("address is " + tracker_servers[i]);
//        }
        tracker_servers[0] = new InetSocketAddress(trackerServerHost, trackerServerPort);
        System.out.println("resolved tracker address is " + tracker_servers[0]);
        g_tracker_group = new TrackerGroup(tracker_servers);

        g_tracker_http_port = trackerHttpPort;
        g_anti_steal_token = FastDFSConfigure.DevConfig.anti_steal_token;
        if (g_anti_steal_token) {
            g_secret_key = FastDFSConfigure.DevConfig.secret_key;
        }

        if (null != storageUploadHost && 0 < storageUploadHost.trim().length() && 0 < storageUploadPort) {
            g_custom_storage_upload = true;
            g_custom_storage_upload_host = storageUploadHost;
            g_custom_storage_upload_port = storageUploadPort;
        }

        if (null != storageDownloadHost && 0 < storageDownloadHost.trim().length() && 0 < storageDownloadPort) {
            g_custom_storage_download = true;
            g_custom_storage_download_host = storageDownloadHost;
            g_custom_storage_download_port = storageDownloadPort;
        }
        g_custom_storage_download_prefix = customStoragePrefixForDownload;
    }
