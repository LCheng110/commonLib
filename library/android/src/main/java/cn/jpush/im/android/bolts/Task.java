 ...
 *
 */
package cn.jpush.im.android.bolts;
...
public class Task<TResult> {
    ...
    public static <TResult> Task<TResult> call(final Callable<TResult> callable,
        ...
        if(executor == BACKGROUND_EXECUTOR){
            ThreadPoolExecutor threadPoolExecutor = (ThreadPoolExecutor) executor;
            Logger.d("bolts", "queue size is " + threadPoolExecutor.getQueue().size());
        }
                    ...
                    if (count.decrementAndGet() == 0) {
                        if (causes.size() != 0) {
                            if (causes.size() == 1) {
                                ...
                                Throwable[] throwables = causes
                                        .toArray(new Throwable[causes.size()]);
     ...
     *                     continuation to be scheduled on a different thread).
     */
     ...
     * completing the task. Essentially, it represents the producer side of a Task<TResult>,
     * providing access to the consumer side through the getTask() method while isolating the Task's
     * completion mechanisms from the consumer.
     */
