...
import com.qiniu.android.jpush.http.PostArgs;
...
final class FormUploader {
	...
	static void upload(HttpManager httpManager, byte[] data, String k,
	...
	static void upload(HttpManager httpManager, File file, String key,
			String token, UpCompletionHandler completionHandler,
			UploadOptions options) {
		post(null, file, key, token, completionHandler, options, httpManager);
	}

	private static void post(byte[] data, File file, String k, String token,
		...
		if (options != null && options.progressHandler != null) {
			progress = new ProgressHandler() {
				...
				public void onProgress(int bytesWritten, int totalSize) {
					...
					Logger.d(TAG, "form upload totalSize = " + totalSize);
					options.progressHandler.onProgressUpdate(percent);
				}
			};
		}
		...
		CompletionHandler completion = new CompletionHandler() {
			...
			public void complete(ResponseInfo info, JSONObject response) {
				if (info.isOK()) {
					if (progress2 != null) {
						options.progressHandler.onProgressUpdate(1.0);
					}
				...
				httpManager.multipartPost("http://" + host, args, progress2,
						retried);
			}
		};

		httpManager.multipartPost("http://" + Config.UP_HOST, args, progress,
				completion);
	}
}
