...
//        Log.d(LOG_TAG, "onProgressData(byte[]) was not overriden, but callback was received");
    ...
    final public void sendProgressDataMessage(byte[] responseBytes,int bytesTotal) {
        sendMessage(obtainMessage(PROGRESS_DATA_MESSAGE, new Object[]{responseBytes,bytesTotal}));
    }
        ...
        switch (message.what) {
            case PROGRESS_DATA_MESSAGE:
                ...
                if (response != null && response.length >= 2) {
                    try {
                        onProgressData((byte[]) response[0],(Integer) response[1]);
                    } catch (Throwable t) {
                ...
                try {
                    ...
                    try {
                        ...
                        while ((l = instream.read(tmp)) != -1 && !Thread.currentThread().isInterrupted()) {
                            ...
                            sendProgressDataMessage(copyOfRange(tmp, 0, l),(int) contentLength);
